	...
	inline void rasterizer<VB, RT>::draw(size_t num_vertexes, size_t vertex_offset)
	{
		...
		while (vertex_id < vertex_offset + num_vertexes)
		{
			...
			float2 bounding_box_begin{
					std::clamp(std::min(std::min(vertices[0].x, vertices[1].x),
												 vertices[2].x),
							   0.f,
							   static_cast<float>(width - 1)),
					std::clamp(std::min(std::min(vertices[0].y, vertices[1].y),
												 vertices[2].y),
							   0.f,
			...
			float2 bounding_box_end{
					std::clamp(std::max(std::max(vertices[0].x, vertices[1].x),
										vertices[2].x),
							   0.f,
							   static_cast<float>(width - 1)),
					std::clamp(std::max(std::max(vertices[0].y, vertices[1].y),
										vertices[2].y),
							   0.f,
							   static_cast<float>(height - 1)),
